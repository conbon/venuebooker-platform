...
then
  ...
  if [ "$TRAVIS_BRANCH" == "staging" ]
  then
    export REACT_APP_USERS_SERVICE_URL="http://venuebooker-platform-staging-alb-579190633.us-east-1.elb.amazonaws.com"
  fi

  if [ "$TRAVIS_BRANCH" == "production" ]
  then
    export REACT_APP_USERS_SERVICE_URL="http://venuebooker-platform-production-alb-453010484.us-east-1.elb.amazonaws.com"
